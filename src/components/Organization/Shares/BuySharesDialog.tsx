...
export function BuySharesDialog({ defaultMemberId, open, onOpenChange, onSuccess }: Props) {
  ...
  const onSubmit = async (values: FormFields) => {
    ...
    try {
      ...
      if ('error' in result) {
        ...
        showToast({ title: 'Success', message: 'Share purchase initiated', type: 'success' })
        form.reset()
        onSuccess()
      }
    } finally {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col gap-4'>
            ...
            <div className='flex justify-end gap-2 pt-2'>
              ...
              <Button type='submit' disabled={isSubmitting}>
                {isSubmitting ? <Spinner /> : 'Buy Shares'}
              </Button>
            </div>
          </form>
        </Form>
